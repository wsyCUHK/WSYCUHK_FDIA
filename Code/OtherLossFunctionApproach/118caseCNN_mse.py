# -*- coding: utf-8 -*-
"""
Created on Wed May 30 00:25:08 2018
...
def weight_loss(a,b):
    return (0.8*losses.binary_crossentropy(a,b)+0.2*losses.mean_squared_error(a,b))
...
reduce_lr=keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=2, verbose=0, mode='auto', min_lr=0.00001)

import numpy as np
model.fit(np.expand_dims(x_train,axis=2), y_train, batch_size=100, epochs=100,callbacks=[reduce_lr])
...
print(row,acca)
sio.savemat('./data/118caseresult_weighted_v5', {'output_mode':pred_y,'output_mode_pred': y_test})