# -*- coding: utf-8 -*-
"""
Created on Mon May  7 11:26:18 2018
...
nameset=[]
nameset.append('./data/4layer128643232Prediction_1') 
# A 4-layer CNN, the number of conv neruals are 128, 64, 32 ,and 32, respectively.
#In our simulation, the pooling process do not improve the performance. Thus, there is no pooling layer.
nameset.append('./data/gg128Prediction_1')
nameset.append('./data/gg80Prediction_1')
nameset.append('./data/3layer12812864Prediction_1')
#nameset.append('./data/0506Prediction_1')
#This is a 2-layer CNN with pooling layer after each convlution layre
nameset.append('./data/3layerPoolingPrediction_1')
nameset.append('./data/5layer6432323232Prediction_1')

nameset.append('./data/133Prediction_1')
nameset.append('./data/233Prediction_1')
nameset.append('./data/333Prediction_1')


#nameset.append('./data/ggsimidaPrediction_1')
nameset.append('./data/gg128Prediction_1')
nameset.append('./data/gg80Prediction_1')
nameset.append('./data/gg02Prediction_1')
nameset.append('./data/0506Prediction_1')


nameset.append('./data/5layer12832323232Prediction_1')
nameset.append('./data/5layer3232323232Prediction_1')
nameset.append('./data/5layer1286432323232Prediction_1')
#nameset.append('./data/5layer6464643232Prediction_1')
# =============================================================================
...
for classindex in range(n_classifiers):
        name=nameset[classindex]
...
for thr in range(20):        
    for i in range(n_pts):
        for j in range(n_labels):
            if votes[i][j]>(thr+1)/10:
                votes[i][j]=1
            else:
                votes[i][j]=0

    e1,e2=cal_acc(votes,attack1)            
    print('Boosting', (thr+1)/10,'|Accuracy:',e2,'Row Accuracy:',e1,'\n')
        # sio.savemat(save_name, {'input_h': X_test/10000000,'output_mode':Y_test,'output_mode_pred': y_pred})
        # # Compute the error for iteration t.
        # error_t = 0.00001  # If we have 0 here, some alphas will be inf.
        # for i in range(n_pts):
